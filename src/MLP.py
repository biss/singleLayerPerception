...
class Network(object):
    ...
    def StochasticGgradientDescent(self, training_data, epochs, mini_batch_size, eta,
            test_data=None):
        ...
        for j in xrange(epochs):
            ...
            if test_data:
                print "Epoch {0} Accuracy: {1} / {2}".format(
    ...
    def evaluate(self, test_data):
        """Return the confusion matrix and number of test inputs for which the neural network outputs the correct result.
        ...
        error = np.sum(self.cost_derivative(self.feedforward(x),y))
        print error
        predict = []
        actual = []
...
net.StochasticGgradientDescent(training_data[:5000], 20, 10, 2.8, test_data=test_data)